    ...
    const handlePurchaseScanned = (data)=>{
        ...
        if (rawItems.length > 0) {
            const mappedItems = rawItems.map((it)=>{
                ...
                const amount = parseAIPrice(it.amount) || qty * price;
                return {
                    productId: matchedProd?.id || '',
                    name: matchedProd?.name || itName || 'Unknown Item',
                    qty,
                    purchasePrice: price || (qty > 0 ? amount / qty : 0),
...
function InventoryPage() {
    ...
    const handleGeminiScanned = (items)=>{
        ...
        items.forEach((item)=>{
            if (!item.name) return;
            const price = parseNum(item.price ?? item.rate ?? 0);
...
//# sourceMappingURL=_7de45fb2._.js.map