...
const getActivityStatus = (daysInactive)=>{
    ...
    if (daysInactive <= 3) return {
        color: '#3B82F6',
        label: `${daysInactive}d ago`,
        icon: '🔵'
    };
    if (daysInactive <= 14) return {
        color: '#F59E0B',
        label: `${daysInactive}d idle`,
        icon: '🟡'
    };
    return {
        color: '#EF4444',
        label: `${daysInactive}d inactive`,
        icon: '🔴'
    };
};
function RegisteredStoresPage() {
    ...
    const fetchStores = async ()=>{
        ...
        try {
            ...
            const list = [];
            const notifs = [];
            let totalPaid = 0, totalActive = 0, newThisWeek = 0;
            const now = Date.now();
            ...
            snap.forEach((docSnap)=>{
                ...
                const userObj = typeof state.user === 'object' && state.user ? state.user : {};
                const plan = userObj.subscriptionType || userObj.plan || 'free';
                const invoices = state.invoices || [];
                const companies = state.companies || [];
                const updatedAt = data.updatedAt ? new Date(data.updatedAt).getTime() : 0;
                const daysSinceActive = updatedAt ? Math.floor((now - updatedAt) / (1000 * 60 * 60 * 24)) : 999;
                const isActive = daysSinceActive <= 7;
                const isNew = updatedAt && now - updatedAt < oneWeek;
                const isPaid = plan !== 'free' && plan !== 'Free Trial' && plan !== 'expired_subscription';
                if (isPaid) totalPaid++;
                if (isActive) totalActive++;
                if (isNew) newThisWeek++;
                // Map companies
                const mappedCompanies = companies.map((co)=>({
                        id: co.id,
                        name: co.name || 'Unnamed',
                        type: co.type || 'N/A',
                        city: co.city || '',
                        invoices: invoices.filter((i)=>i.companyId === co.id).length
                    }));
                const revenue = invoices.reduce((sum, inv)=>sum + (inv.total || 0), 0);
                // Notifications
                if (isNew) notifs.push({
                    id: docSnap.id,
                    msg: `New store registered: ${userObj.name || data.email || docSnap.id}`,
                    type: 'new'
                });
                if (daysSinceActive > 30) notifs.push({
                    id: docSnap.id + '_inactive',
                    msg: `Store inactive 30+ days: ${userObj.name || data.email}`,
                    type: 'inactive'
                });
                if (plan === 'expired_subscription') notifs.push({
                    id: docSnap.id + '_exp',
                    msg: `Expired plan: ${userObj.name || data.email}`,
                    type: 'expired'
                });
                list.push({
                    docId: docSnap.id,
                    email: userObj.email || data.email || 'N/A',
                    name: userObj.name || data.email || 'Unknown',
            ...
            setStores(list.sort((a, b)=>b.daysInactive - a.daysInactive === 0 ? 0 : a.daysInactive - b.daysInactive));
            setStats({
                total: list.length,
                active: totalActive,
                paid: totalPaid,
    ...
    const filtered = stores.filter((s)=>{
        ...
        const matchSearch = !q || s.name.toLowerCase().includes(q) || s.email.toLowerCase().includes(q) || s.phone.includes(q) || s.docId.includes(q);
        const matchFilter = filter === 'all' ? true : filter === 'active' ? s.isActive : filter === 'inactive' ? !s.isActive : filter === 'paid' ? s.plan !== 'free' && s.plan !== 'Free Trial' && s.plan !== 'expired_subscription' : filter === 'free' ? s.plan === 'free' || s.plan === 'Free Trial' : filter === 'new' ? s.daysInactive <= 7 : true;
    ...
    return /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
        ...
        children: [
            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("style", {
                children: `
                .stores-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(140px,1fr)); gap: 16px; margin-bottom: 32px; }
                .filter-bar { display: flex; gap: 8px; flex-wrap: wrap; margin-bottom: 20px; }
                .store-row { background: white; border-radius: 16px; border: 1px solid #F1F5F9; box-shadow: 0 2px 8px rgba(0,0,0,0.04); overflow: hidden; transition: box-shadow 0.2s; }
                .store-row:hover { box-shadow: 0 6px 24px rgba(0,0,0,0.08); }
                .store-header { display: flex; align-items: center; gap: 14px; padding: 16px 20px; cursor: pointer; flex-wrap: wrap; }
                .expand-section { background: #F8FAFC; border-top: 1px solid #F1F5F9; padding: 16px 20px; }
                .stat-chip { padding: 6px 12px; border-radius: 99px; font-size: 11px; font-weight: 700; display: inline-flex; align-items: center; gap: 4px; }
                .notif-panel { position: absolute; right: 0; top: 42px; width: 340px; background: white; border-radius: 16px; border: 1px solid #E2E8F0; box-shadow: 0 16px 48px rgba(0,0,0,0.12); z-index: 200; overflow: hidden; }
                @media (max-width: 639px) {
                    .stores-grid { grid-template-columns: repeat(2,1fr); }
                    .store-header { gap: 10px; padding: 12px 14px; }
                    .notif-panel { width: 280px; right: 0; }
                }
            `
            }, void 0, false, {
                    ...
                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                        children: [
                            ...
                            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                                ...
                                children: "Monitor all registered stores, activity status, and plan health."
                            }, void 0, false, {
                                fileName: "[project]/app/admin/companies/page.tsx",
                                lineNumber: 182,
                                columnNumber: 21
            ...
            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                ...
                ].map(({ label, value, icon: Icon, bg, color })=>/*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                        style: {
                            background: 'white',
                            borderRadius: 16,
                            padding: '18px 16px',
            ...
            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                ...
                children: [
                    ...
                    'new'
                        ...
                        onClick: ()=>setFilter(f),
                        className: filter === f ? 'btn btn-blue btn-sm' : 'btn btn-outline btn-sm',
                        style: {
                            fontSize: 12,
                            height: 34,
                            textTransform: 'capitalize'
                        },
                        children: f === 'all' ? `All (${stores.length})` : f === 'active' ? `🟢 Active (${stores.filter((s)=>s.isActive).length})` : f === 'inactive' ? `🔴 Inactive (${stores.filter((s)=>!s.isActive).length})` : f === 'paid' ? `⭐ Paid (${stores.filter((s)=>s.plan !== 'free' && s.plan !== 'Free Trial' && s.plan !== 'expired_subscription').length})` : f === 'free' ? `🆓 Free (${stores.filter((s)=>s.plan === 'free' || s.plan === 'Free Trial').length})` : `🆕 New (${stores.filter((s)=>s.daysInactive <= 7).length})`
                    }, f, false, {
                        fileName: "[project]/app/admin/companies/page.tsx",
                        lineNumber: 249,
                        columnNumber: 21
            ...
            loading ? /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                ...
                children: [
                    ...
                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                        children: "No stores match your criteria."
                    }, void 0, false, {
                        fileName: "[project]/app/admin/companies/page.tsx",
                        lineNumber: 267,
                        columnNumber: 21
                ...
                children: filtered.map((store)=>{
                    const planBadge = getPlanBadge(store.plan);
                    const activityStatus = getActivityStatus(store.daysInactive);
                    const isExpanded = expandedId === store.docId;
                    return /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                        className: "store-row",
                        children: [
                            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                className: "store-header",
                                onClick: ()=>setExpandedId(isExpanded ? null : store.docId),
                                children: [
                                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                        style: {
                                            width: 44,
                                        ...
                                        children: [
                                            ...
                                            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                ...
                                                children: [
                                                    ...
                                                    store.phone !== 'N/A' && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                                                        style: {
                                                            fontSize: 11,
                                                            color: '#64748B',
                                                            display: 'flex',
                                    ...
                                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                        ...
                                        children: [
                                            ...
                                            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                                                className: "stat-chip",
                                                style: {
                                                    background: activityStatus.color + '18',
                                                    color: activityStatus.color
                                                },
                                                children: [
                                                    store.isActive ? /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])(__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$lucide$2d$react$2f$dist$2f$esm$2f$icons$2f$wifi$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__$3c$export__default__as__Wifi$3e$__["Wifi"], {
                                                        size: 11
                                                    }, void 0, false, {
                                                        fileName: "[project]/app/admin/companies/page.tsx",
                                                        lineNumber: 301,
                            ...
                            isExpanded && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                ...
                                children: [
                                            ...
                                            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                ...
                                                children: [
                                                    ...
                                                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                                                        ...
                                                        children: store.lastActive ? new Date(store.lastActive).toLocaleDateString('en-IN', {
                                                            day: '2-digit',
                                                            month: 'short',
                                                            year: 'numeric'
                                                        }) : 'Unknown'
                                    ...
                                    store.companies.length === 0 && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                        style: {
                                            background: '#FFFBEB',
                                            padding: '10px 14px',
                                            borderRadius: 10,
                                        ...
                                        children: "⚠️ No companies created yet under this account."
                                    }, void 0, false, {
                                        fileName: "[project]/app/admin/companies/page.tsx",
                                        lineNumber: 351,
                                        columnNumber: 45
...
_s(RegisteredStoresPage, "Qw3IRw3lZKpWXtpgMNEUZ6Y+Mh0=");
...
//# sourceMappingURL=_56874c88._.js.map